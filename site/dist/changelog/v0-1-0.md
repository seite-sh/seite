...
title: v0.1.0
date: 2026-02-20
description: Initial public release of page — the AI-native static site generator built in Rust.
tags:
- new
---
...
- AI-generated custom themes via `page theme create "<description>"` (requires Claude Code)
- Theme install from URL (`page theme install <url>`) and export for sharing (`page theme export <name>`)
...
- `page agent` spawns Claude Code with full site context — content inventory, frontmatter format, template list, available commands
- `page agent "prompt"` for non-interactive one-shot content generation
- MCP server (`page mcp`) with 5 tools and 6+ resources for AI tool integration, auto-started by Claude Code
- Every site ships `llms.txt`, `llms-full.txt`, and raw markdown alongside HTML
- `page init` generates `.claude/CLAUDE.md` with comprehensive site context for AI agents

## Deploy

- GitHub Pages, Cloudflare Pages, and Netlify — all built in
- Guided setup via `page deploy --setup` (creates repos/projects, configures auth, generates CI workflows)
...
- Multi-site workspaces (`page-workspace.toml`) with unified dev server and per-site deploy
- Collection management: `page collection add <preset>` and `page collection list`
- Project metadata and upgrades: `.page/config.json` tracks version, `page upgrade` applies version-gated changes
