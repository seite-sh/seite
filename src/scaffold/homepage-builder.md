...
When the user asks you to create, build, or redesign their homepage or landing page, follow this guided process. The goal is to produce two files: `content/pages/index.md` (data) and `templates/index.html` (layout). Iterate with the user until they are happy.

### Phase 1: Discovery

Before writing any code, ask the user these questions (adapt based on what you already know from the site config and existing content):
...
If the user gives a short answer like "just make me a landing page for my SaaS", infer reasonable defaults and present your plan before building. Do not ask all questions at once if context makes some answers obvious.

### Phase 2: Content Plan

Present a section-by-section outline to the user before writing files. Format it like this:
...
**`content/pages/index.md`** — All content lives here as `extra:` frontmatter fields. Keep the markdown body empty (or use it for a simple prose section if appropriate). Structure your `extra:` fields with a consistent naming pattern per section:
...
**`templates/index.html`** — Tera template that extends `base.html`. Uses `{{ page.extra.* }}` to render each section. Must include:
...
- Collection listings at the bottom if the site has posts/docs (use the standard `{% for collection in collections %}` loop)
- Responsive design (mobile-first, with breakpoints at 600px and 900px)
- Respect the current theme's CSS custom properties (e.g., `var(--heading)`, `var(--text)`, `var(--bg)`, `var(--green)`, `var(--border)`) when available
...
CSS guidelines for the template:
...
- **Theme mismatch** — If the homepage fights the base theme, suggest `seite theme apply <name>` or adjustments to `base.html`
...
When building sections, use these proven patterns. Adapt to the site's content — do not use placeholder copy.
