...
- **Image processing** — Add `[images]` to `page.toml` to auto-resize images, generate WebP variants, inject `srcset`/`<picture>` elements, and add `loading="lazy"`. See Configuration section below.
- **Analytics** — Add `[analytics]` to `page.toml` for Google Analytics, GTM, Plausible, Fathom, or Umami. Optional cookie consent banner. See Configuration section below.
